...
CREATE OR REPLACE VIEW qgep.vw_wizard_cover_manhole_sige AS 
 SELECT co.obj_id,
    co.brand,
    co.cover_shape,
    co.diameter,
   ...
   FROM qgep.vw_cover co
     LEFT JOIN qgep.vw_manhole_sige mh ON mh.obj_id::text = co.fk_wastewater_structure::text
     LEFT JOIN qgep.vw_wastewater_node wn ON wn.fk_wastewater_structure::text = mh.obj_id::text
  WHERE false;

-- INSERT function

CREATE OR REPLACE FUNCTION qgep.vw_wizard_cover_manhole_sige_INSERT()
  RETURNS trigger AS
$BODY$
DECLARE
  mh_obj_id character varying(16);
BEGIN
  INSERT INTO qgep.vw_manhole_sige(
         dimension1
       , dimension2
       , depth
       , function
...
DROP TRIGGER IF EXISTS vw_wizard_cover_manhole_ON_INSERT ON qgep.vw_wizard_cover_manhole_sige;

CREATE TRIGGER vw_wizard_cover_manhole_ON_INSERT INSTEAD OF INSERT ON qgep.vw_wizard_cover_manhole_sige
  FOR EACH ROW EXECUTE PROCEDURE qgep.vw_wizard_cover_manhole_sige_INSERT();
